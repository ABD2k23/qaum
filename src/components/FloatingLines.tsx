...
import {
  ...
  Vector2,
  Clock,
  Color, // Using Three.js Color class for easier parsing
} from "three";
...
float wave(vec2 uv, float offset, vec2 screenUv, vec2 mouseUv, bool shouldBend) {
  ...
  if (shouldBend) {
    ...
    float influence = exp(-dot(d, d) * (bendRadius * 0.1));
    float bendOffset = (mouseUv.y - screenUv.y) * influence * (bendStrength * 0.1) * bendInfluence;
    y += bendOffset;
  }
...
type FloatingLinesProps = {
  color?: string; // New prop for single color
...
export default function FloatingLines({
  color = "rgb(179, 60, 70)", // Default to your red
  linesGradient,
  ...
  useEffect(() => {
    ...
    // Convert string color to THREE Color for normalization
    const baseThreeColor = new Color(color);
    ...
    const uniforms = {
      ...
      lineGradient: {
        value: Array.from(
          ...
          () =>
            new Vector3(baseThreeColor.r, baseThreeColor.g, baseThreeColor.b),
        ),
      },
